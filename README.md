...
- `language_tools/qa.py` — 轻量 QA 层（空值/长度比异常/重复TU/数字不匹配），`convert(..., qa=True)` 时启用，csv 增加 confidence/status/issues 列（默认关闭，不影响 Phase 1 基线）

对照 Phase 0 回归基线（`tests/fixtures/`）验证：csv 输出与移植前的参考脚本逐字节一致；sdltm/tmx 输出语义等价（guid/时间戳除外，两者在写入时都是运行时生成的，见 DESIGN.md 第5节）。每个新 reader 都补了语言方向反转测试（DESIGN.md 6.4 强制要求）。
...
- Phase 3：tmx/sdltm 语料库互转 reader，SDLTM Level 2 兼容目标
