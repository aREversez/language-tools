...
def _lang_base(tag):
    """Primary language subtag, e.g. 'en-US' -> 'en', 'zh-Hans-CN' -> 'zh'."""
    return tag.split('-', 1)[0].lower() if tag else ''


def _lang_matches(requested, actual):
    """Case-insensitive match that also tolerates a region-less tag on
    either side -- e.g. requested='en-US' matches actual='en' (some CAT
    tools export TMX with a bare ``xml:lang="en"`` and no region), and
    requested='en' matches actual='en-GB'. Confirmed empirically: real
    TMX exports lacking a region subtag were silently skipped entirely
    (0 units) before this fallback, because the old code compared the
    two tags for exact equality only.
    """
...
def read(path, src_lang=None, tgt_lang=None, **opts):
    ...
    Matching tries an exact tag comparison first, then falls back to a
    base-subtag comparison (see ``_lang_matches``) so a request for
    'en-US' still matches a tuv tagged just 'en' -- some TMX exports omit
    the region subtag entirely.
    """
    ...
    for tu in body:
        ...
        if src_lang and tgt_lang:
            # Exact match first (so 'en-US' vs 'en-GB' in the same <tu>
            # picks the right one when both are present); fall back to
            # base-subtag match (see _lang_matches) only when no tuv has
            # the exact tag -- covers TMX exports that dropped the region.
            src_tuv = next((t for t in tuvs if _tuv_lang(t).lower() == src_lang.lower()), None)
            if src_tuv is None:
                src_tuv = next((t for t in tuvs if _lang_matches(src_lang, _tuv_lang(t))), None)
            tgt_tuv = next((t for t in tuvs if _tuv_lang(t).lower() == tgt_lang.lower()), None)
            if tgt_tuv is None:
                tgt_tuv = next((t for t in tuvs if _lang_matches(tgt_lang, _tuv_lang(t))), None)
